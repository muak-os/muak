...
use crate::model::{DhcpLease, Ipv6Config};
            ...
            3 => {
                ...
                if opt_len >= 12 {
                    ...
                    while sub_pos + 4 <= opt_len {
                        let sub_code =
                            u16::from_be_bytes([opt_data[sub_pos], opt_data[sub_pos + 1]]);
                        let sub_len = u16::from_be_bytes([opt_data[sub_pos + 2], opt_data[sub_pos + 3]])
                            as usize;
                        sub_pos += 4;
                        ...
                        if sub_code == 5 && sub_len >= 24 {
                            ...
                            let addr_bytes: [u8; 16] =
                                opt_data[sub_pos..sub_pos + 16].try_into()?;
...
async fn dhcpv6_full_handshake(
    ...
    let offered_addr = advertise
        .address
        .ok_or_else(|| anyhow::anyhow!("No address in ADVERTISE"))?;

    kmsg::info!(@ "networkd", "DHCPv6: got ADVERTISE address={}", offered_addr);
    ...
    kmsg::info!(@ "networkd", "DHCPv6: got REPLY address={}", address);
...
async fn retry_send_receive(
        ...
        match timeout(Duration::from_secs(current_timeout), socket.recv_from(&mut buf)).await {
            Ok(Ok((len, _))) => {
                ...
                if response.xid != expected_xid {
                    kmsg::info!(@ "networkd", "DHCPv6: XID mismatch, ignoring");
                    continue;
                }
                ...
                if response.msg_type != expected_msg_type {
                    kmsg::info!(
                        @ "networkd",
                        "DHCPv6: unexpected message type {}, expected {}",
                        response.msg_type,
                        expected_msg_type
                    );
                    continue;
            ...
            Ok(Err(e)) => {
                kmsg::info!(@ "networkd", "DHCPv6: receive error: {}", e);
            }
            Err(_) => {
                kmsg::info!(@ "networkd", "DHCPv6: timeout waiting for response");
            }
        }
    ...
    anyhow::bail!(
        "DHCPv6: {} failed after {} attempts",
        msg_name,
        DHCPV6_MAX_RETRIES
    )
}

/// Run DHCPv6 client on an interface to acquire an IPv6 address
pub async fn run_dhcpv6_client(interface: &str, mac: &[u8; 6]) -> Result<(Ipv6Config, DhcpLease)> {
    kmsg::info!(@ "networkd", "DHCPv6: starting on {}", interface);
...
mod tests {
    ...
    fn test_build_request_message_type() {
        ...
        let msg =
            build_request_message(0xABCDEF, &client_duid, &server_duid, 1, addr, 3600, 7200)
                .unwrap();
    ...
    fn test_build_request_contains_server_id() {
        ...
        let msg =
            build_request_message(0x123456, &client_duid, &server_duid, 42, addr, 1800, 3600)
                .unwrap();
    ...
    fn test_build_request_contains_ia_address() {
        ...
        let msg_str = msg
            ...
            .map(|b| format!("{:02x}", b))
            .collect::<String>();
        let addr_str = addr_bytes
            .iter()
            .map(|b| format!("{:02x}", b))
            .collect::<String>();

        assert!(
            msg_str.contains(&addr_str),
            "REQUEST must contain the requested address"
        );
    }
