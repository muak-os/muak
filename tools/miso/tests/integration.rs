...
mod tests {
    ...
    fn build_iso_output_is_sector_aligned() {
        ...
        // ASSERT
        assert_eq!(
            iso.len().rem_euclid(SECTOR_SIZE),
            0,
    ...
    fn build_iso_mbr_boot_signature_present() {
        ...
        // ASSERT
        assert_eq!(byte_at(&iso, 510), 0x55, "MBR byte 510 must be 0x55");
        assert_eq!(byte_at(&iso, 511), 0xAA, "MBR byte 511 must be 0xAA");
    }
    ...
    fn build_iso_boot_catalog_has_55aa_keys() {
        ...
        let cat_start = SECTOR_SIZE * 21;
        assert_eq!(
            byte_at(&iso, cat_start + 30),
            0x55,
            "El Torito key byte 1 must be 0x55"
        );
        assert_eq!(
            byte_at(&iso, cat_start + 31),
            0xAA,
    ...
    fn build_raw_has_protective_mbr() {
        ...
        // ASSERT
        assert_eq!(byte_at(&img, 510), 0x55, "MBR byte 510 must be 0x55");
        assert_eq!(byte_at(&img, 511), 0xAA, "MBR byte 511 must be 0xAA");
        assert_eq!(
            byte_at(&img, 450),
