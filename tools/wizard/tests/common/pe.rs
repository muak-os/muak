//! Minimal PE64 EFI stub generator.
...
/// Generates a minimal valid PE64 EFI stub (same shape yuki's tests use).
pub fn generate_stub() -> Vec<u8> {
    generate_stub_with_section_count(1)
}

fn generate_stub_with_section_count(n: u16) -> Vec<u8> {
    ...
    write_u32(&mut pe, 0x3C, u32::try_from(DOS_HEADER_SIZE).unwrap_or(0));
...
fn write_optional_header(
    ...
    off = off.saturating_add(2);
    write_u32(buf, off, u32::try_from(section_size).unwrap_or(0));
    off = off.saturating_add(4);
    off = off.saturating_add(8);
    write_u32(buf, off, u32::try_from(SECTION_ALIGNMENT).unwrap_or(0));
    off = off.saturating_add(4);
    write_u32(buf, off, u32::try_from(SECTION_ALIGNMENT).unwrap_or(0));
    off = off.saturating_add(4);
    ...
    write_u32(buf, off, u32::try_from(SECTION_ALIGNMENT).unwrap_or(0));
    off = off.saturating_add(4);
    write_u32(buf, off, u32::try_from(FILE_ALIGNMENT).unwrap_or(0));
    off = off.saturating_add(4);
    off = off.saturating_add(16);
    write_u32(
        buf,
        off,
        u32::try_from(SECTION_ALIGNMENT.saturating_mul(2)).unwrap_or(0),
    );
    off = off.saturating_add(4);
...
fn write_text_section_header(buf: &mut [u8], optional_header_offset: usize) {
    ...
    write_u32(
        ...
        u32::try_from(512).unwrap_or(0),
    );
    write_u32(
        buf,
        section_headers_offset.saturating_add(12),
        u32::try_from(4096).unwrap_or(0),
    );
    write_u32(
        buf,
...
fn write_extra_section_header(buf: &mut [u8], section_headers_offset: usize, index: usize) {
    let section_header_offset = section_headers_offset.saturating_add(index.saturating_mul(40));
    write_bytes(buf, section_header_offset, b".null");
    write_u32(buf, section_header_offset.saturating_add(8), 1);
    let virtual_address = u32::try_from(index.saturating_add(1).saturating_mul(4096)).unwrap_or(0);
    write_u32(
        buf,
...
fn write_extra_section_headers(buf: &mut [u8], optional_header_offset: usize, num_sections: u16) {
    let section_headers_offset = optional_header_offset.saturating_add(240);
    (1..usize::from(num_sections)).for_each(|index| {
        write_extra_section_header(buf, section_headers_offset, index);
    });
}
...
fn write_pe_headers(buf: &mut [u8], coff_offset: usize, num_sections: u16) {
    ...
    write_u16(buf, off, u16::try_from(240).unwrap_or(0));
    off = off.saturating_add(2);
