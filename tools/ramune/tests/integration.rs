...
fn create_writes_expected_archive_entries() {
    ...
    // ACT
    ramune::create(&create_config(&init, &rootfs, None), &output).expect("create should succeed");
...
async fn extend_with_extensions_appends_named_archive() {
    ...
    assert_eq!(extension_entry.mode, 0o100644);
    assert!(
        !extension_entry.data.is_empty(),
        "extension erofs should not be empty"
    );
    assert_eq!(
        extension_entry.data.len() % 4096,
        0,
