...
pub fn write(files: &mut [EspFile], root: &Path) -> Result<()> {
...
fn copy_to_file(file: &mut EspFile, dest: &Path) -> Result<()> {
...
mod tests {
    ...
    fn populate_streams_files_into_mounted_dir() {
        ...
        let data = b"arm_64bit=1".to_vec();
        let mut spec = EspSpec::builder()
            .add_file(fat_boot(b"uki-payload"))
