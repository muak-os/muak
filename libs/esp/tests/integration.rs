...
fn public_api_populate_writes_all_files_to_directory() {
    ...
    // ASSERT
    assert_eq!(
        std::fs::read(temp_dir.path().join("EFI/BOOT/BOOTX64.EFI")).expect("boot file must exist"),
        b"uki"
    );
    assert_eq!(
...
fn public_api_populate_propagates_write_errors() {
    ...
    let temp_dir = tempfile::tempdir().expect("temp dir must be created");
    std::fs::create_dir(temp_dir.path().join("existing-dir")).expect("directory must exist");
