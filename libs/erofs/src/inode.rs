...
pub fn write_compact(buf: &mut [u8], params: &CompactInodeParams) {
    debug_assert!(
        buf.len() >= COMPACT_INODE_SIZE,
        "compact inode buffer must be at least {COMPACT_INODE_SIZE} bytes"
    );

    let inode_format = encode_i_format_compact(params.datalayout);
    let wrote_all = write_bytes(buf, 0x00, &inode_format.to_le_bytes())
        && write_bytes(buf, 0x02, &params.xattr_icount.to_le_bytes())
        && write_bytes(buf, 0x04, &params.mode.to_le_bytes())
        && write_bytes(buf, 0x06, &params.nlink.to_le_bytes())
        && write_bytes(buf, 0x08, &params.size.to_le_bytes())
        && write_bytes(buf, 0x0C, &0_u32.to_le_bytes())
        && write_bytes(buf, 0x10, &params.startblk_or_rdev.to_le_bytes())
        && write_bytes(buf, 0x14, &params.ino.to_le_bytes())
        && write_bytes(buf, 0x18, &params.uid.to_le_bytes())
        && write_bytes(buf, 0x1A, &params.gid.to_le_bytes())
        && write_bytes(buf, 0x1C, &params.reserved2.to_le_bytes());
...
pub fn slot_count(inode_size: usize, xattr_size: usize, inline_size: usize) -> usize {
    let total = inode_size
        .saturating_add(xattr_size)
        .saturating_add(inline_size);
