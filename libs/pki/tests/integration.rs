...
fn csr_sign_rejects_tampered_signature() -> Result<()> {
    ...
    let mut request = x509_cert::request::CertReq::from_pem(&csr_pem)?;
    let mut signature_bytes = request
        .signature
        .as_bytes()
        .ok_or(PkiError::CsrVerification)?
        .to_vec();
    let last_index = signature_bytes.len().saturating_sub(1);
    signature_bytes[last_index] ^= 0x01;
...
fn build_server_certificate(dns_names: Vec<String>) -> Result<Certificate> {
    ...
    let profile = pki::profile::MuakServer {
...
#[test]
fn profile_ca_and_client_profile_subject_and_issuer_accessors_return_expected_names() -> Result<()>
{
    // ARRANGE
    let subject = Name::from_str("CN=profile-subject,O=Muak")?;
    let issuer = Name::from_str("CN=profile-issuer,O=Muak")?;
    let ca_profile = pki::profile::MuakCa {
        subject: subject.clone(),
    };
    let client_profile = pki::profile::MuakClient {
        issuer: issuer.clone(),
        subject: subject.clone(),
    };

    // ACT
    let ca_issuer = x509_cert::builder::profile::BuilderProfile::get_issuer(&ca_profile, &subject);
    let ca_subject = x509_cert::builder::profile::BuilderProfile::get_subject(&ca_profile);
    let client_issuer =
        x509_cert::builder::profile::BuilderProfile::get_issuer(&client_profile, &subject);
    let client_subject = x509_cert::builder::profile::BuilderProfile::get_subject(&client_profile);
...
fn profile_client_profile_builds_client_auth_extended_key_usage() -> Result<()> {
    ...
    let profile = pki::profile::MuakClient {
        issuer: ca_certificate.tbs_certificate().subject().clone(),
