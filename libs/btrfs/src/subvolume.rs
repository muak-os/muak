//! Subvolume management for btrfs filesystems.

use std::path::PathBuf;

use rustix::fs::{Mode, OFlags, open};
use rustix::ioctl::{Setter, ioctl};

use crate::error::{BtrfsError, Result};
use crate::ioctl::{BTRFS_IOC_SNAP_DESTROY, BTRFS_IOC_SUBVOL_CREATE, BTRFS_PATH_NAME_MAX, VolArgs};

/// Create a Btrfs subvolume for a specific VM.
///
/// # Arguments
/// * `vm_id` - The VM/subvolume identifier
/// * `data_dir` - Data directory where subvolume will be created
///
/// # Returns
/// The path to the created subvolume on success.
///
/// # Errors
/// Returns an error if the containing directory cannot be opened or the ioctl fails.
pub fn create(vm_id: &str, data_dir: &str) -> Result<PathBuf> {
    let path = PathBuf::from(data_dir).join(vm_id);

    let file = open(data_dir, OFlags::RDONLY | OFlags::DIRECTORY, Mode::empty())?;

    let mut args = VolArgs {
        fd: -1,
        name: [0_u8; BTRFS_PATH_NAME_MAX + 1],
    };

    copy_name(&mut args.name, vm_id);

    // SAFETY: `BTRFS_IOC_SUBVOL_CREATE` expects `VolArgs` by value.
    let setter = unsafe { Setter::<BTRFS_IOC_SUBVOL_CREATE, VolArgs>::new(args) };

    // SAFETY: The directory file descriptor is valid and the ioctl argument type matches.
    unsafe { ioctl(&file, setter) }.map_err(|source| BtrfsError::Subvolume {
        operation: "create",
        path: path.clone(),
        source,
    })?;

    Ok(path)
}

/// Delete a Btrfs subvolume for a specific VM.
///
/// # Arguments
/// * `vm_id` - The VM/subvolume identifier
/// * `data_dir` - Data directory where subvolume is located
///
/// # Errors
/// Returns an error if the ioctl fails.
pub fn delete(vm_id: &str, data_dir: &str) -> Result<()> {
    let path = PathBuf::from(data_dir).join(vm_id);

    if !path.exists() {
        return Ok(());
    }

    let file = open(data_dir, OFlags::RDONLY | OFlags::DIRECTORY, Mode::empty())?;

    let mut args = VolArgs {
        fd: -1,
        name: [0_u8; BTRFS_PATH_NAME_MAX + 1],
    };

    copy_name(&mut args.name, vm_id);

    // SAFETY: `BTRFS_IOC_SNAP_DESTROY` expects `VolArgs` by value.
    let setter = unsafe { Setter::<BTRFS_IOC_SNAP_DESTROY, VolArgs>::new(args) };

    // SAFETY: The directory file descriptor is valid and the ioctl argument type matches.
    unsafe { ioctl(&file, setter) }.map_err(|source| BtrfsError::Subvolume {
        operation: "delete",
        path: path.clone(),
        source,
    })?;

    Ok(())
}

/// List subvolumes by reading directories in the data directory.
///
/// # Arguments
/// * `data_dir` - Data directory to list subvolumes from
///
/// # Returns
/// A vector of subvolume names.
///
/// # Errors
/// Returns an error if the data directory cannot be read.
pub fn list(data_dir: &str) -> Result<Vec<String>> {
    let mut vm_ids = Vec::new();

    let entries = std::fs::read_dir(data_dir)?;

    for entry in entries {
        let entry = entry?;
        let path = entry.path();

        if path.is_dir()
            && let Some(name) = path.file_name()
            && let Some(name_str) = name.to_str()
            && !name_str.starts_with('.')
        {
            vm_ids.push(name_str.to_owned());
        }
    }

    Ok(vm_ids)
}

fn copy_name(dest: &mut [u8; BTRFS_PATH_NAME_MAX + 1], name: &str) {
    let name_bytes = name.as_bytes();
    let copy_len = name_bytes.len().min(BTRFS_PATH_NAME_MAX);

    if let (Some(dest), Some(src)) = (dest.get_mut(..copy_len), name_bytes.get(..copy_len)) {
        dest.copy_from_slice(src);
    }
}
