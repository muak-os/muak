...
use crate::types::{
    ATTR_ARCHIVE, ATTR_DIRECTORY, ATTR_LFN, ClusterMap, FatKind, FatLayout, FileSource,
    ROOT_CLUSTER, fat12_16_cluster, fat32_cluster,
};
...
pub(crate) fn build_data(
    ...
    for (file_index, ef) in files.iter().enumerate() {
        let fp = std::path::Path::new(ef.path());
        ...
        let size = u32::try_from(ef.size()).unwrap_or(u32::MAX);
        let idx = data.len().next_multiple_of(32);
        data.resize(idx, 0);
