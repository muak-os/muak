...
pub(super) fn resolve_key(device: &str) -> Result<Option<Passphrase>> {
...
fn try_tpm2_unseal(device: &str) -> Result<Option<Passphrase>> {
...
fn parse_luks_key() -> Option<Passphrase> {
    let cmdline = std::fs::read_to_string("/proc/cmdline").ok()?;

    parse_luks_key_from_cmdline(&cmdline)
}

fn parse_luks_key_from_cmdline(cmdline: &str) -> Option<Passphrase> {
    let token = cmdline
        .split_whitespace()
...
mod tests {
    ...
    fn parse_luks_key_from_cmdline_decodes_valid_key() {
        // ARRANGE
        let cmdline = "quiet luks.key=c2VjcmV0LWtleS1kYXRh splash";
    ...
    fn parse_luks_key_from_cmdline_picks_first_matching_token() {
        // ARRANGE
        let cmdline = "luks.key=Zmlyc3Q luks.key=c2Vjb25k";
    ...
    fn parse_luks_key_from_cmdline_rejects_garbled_suffix() {
        // ARRANGE
        let cmdline = "quiet luks.key=c2VjcmV0LWtleS1kYXRhgarbled splash";
