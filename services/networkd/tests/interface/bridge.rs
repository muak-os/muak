...
async fn bridge_port_deconfigured_after_bridge_creation() {
    ...
    wait_for_state(&handle, InterfaceState::Discovered).await;

    // ASSERT
    let port_snap = handle.state_rx.borrow().clone();
    assert_eq!(port_snap.state, InterfaceState::Discovered);
    assert!(
